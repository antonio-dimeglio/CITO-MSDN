...
def main():
    ...
    clean_image = imageio.v2.imread(output_dir / "train" / "clean" / "0.tiff")
    label = imageio.v2.imread(output_dir / "train" / "label" / "0.tiff")
    noisy_image = imageio.v2.imread(output_dir / "train" / "noisy" / "0.tiff")

    fig, ax = plt.subplots(1, 3, figsize=(15, 5))
    ax[0].imshow(clean_image, cmap="gray")
    ax[0].set_title("Clean Image")
    ax[0].axis("off")
    ax[1].imshow(label, cmap="tab10")
